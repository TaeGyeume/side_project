...
const AccommodationForm = ({ onSubmit, initialData = {}, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const lng = coordinates.lng || 126.9780;
    ...
    requestData.append("coordinates", JSON.stringify({
      type: "Point",
      coordinates: [lng, lat]
    }));
  ...
  return (
    <form onSubmit={handleSubmit} className="accommodation-form">
      ...
      <div>
        ...
        <input type="number" name="lng" step="any" value={formData.coordinates.lng} onChange={handleCoordinateChange} required />
      </div>
      <div>
        <label>위도 (Latitude)</label>
        <input type="number" name="lat" step="any" value={formData.coordinates.lat} onChange={handleCoordinateChange} required />
      </div>
      ...
      <div>
        ...
        {formData.amenities.map((amenity, index) => (
          <div key={index}>
            <input type="text" value={amenity} onChange={(e) => handleAmenityChange(index, e.target.value)} placeholder="편의시설 입력" />
            <button type="button" onClick={() => handleRemoveAmenity(index)}>X</button>
          </div>
        ))}
        <button type="button" onClick={handleAddAmenity}>+ 추가</button>
      </div>
