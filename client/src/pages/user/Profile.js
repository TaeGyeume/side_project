...
const Profile = () => {
  ...
  const handleCheckDuplicate = async () => {
    ...
    try {
      const response = await authAPI.checkDuplicate({ [editingField]: userData[editingField] });
      setCheckMessage({ ...checkMessage, [editingField]: response.message });
    } catch (error) {
      setCheckMessage({
        ...checkMessage,
        [editingField]: error.response?.data?.message || error.message || "중복 확인에 실패했습니다.",
      });
    }
  };
          ...
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{fieldNames[editingField] || editingField} 수정</h5>
              </div>
              <div className="modal-body">
                ...
                {["userid", "email", "phone"].includes(editingField) && (
                  <button className="btn btn-outline-secondary mt-2 w-100" onClick={handleCheckDuplicate}>
                    중복 확인
                  </button>
                )}
