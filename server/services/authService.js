...
exports.loginUser = async ({ userid, password }, res) => {
  ...
  const accessToken = jwt.sign({ id: user._id, roles: user.roles }, process.env.JWT_SECRET, {
    expiresIn: '7d'
  });

  const refreshToken = jwt.sign({ id: user._id, roles: user.roles }, process.env.REFRESH_TOKEN_SECRET, {
    expiresIn: '7d'
  });
...
exports.refreshAccessToken = async (refreshToken, res) => {
  try {
    ...
    const newAccessToken = jwt.sign({ id: decoded.id, roles: decoded.roles }, process.env.JWT_SECRET, {
      expiresIn: '7d'
    });

    const newRefreshToken = jwt.sign({ id: decoded.id, roles: decoded.roles }, process.env.REFRESH_TOKEN_SECRET, {
      expiresIn: '7d'
    });
