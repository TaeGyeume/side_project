...
exports.deleteFollow = async (req, res) => {
  ...
  try {
    ...
    if (!deletedFollow) {
      console.error(`Follow with ID ${followId} not found.`);
...
exports.getAllUsersWithFollowStatus = async (req, res) => {
  ...
  try {
    ...
    followings.forEach((f) => {
      followingMap[f.followingId] = f.status;
    });

    const usersWithStatus = allUsers.map((user) => ({
      ...user,
      isFollowing: followingMap[user._id] === "ACCEPTED",
      status: followingMap[user._id] || null, // PENDING 등 상태 포함
    }));
